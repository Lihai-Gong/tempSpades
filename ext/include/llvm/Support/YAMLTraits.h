...
yamlize(IO &io, T &Val, bool, Context &Ctx) {
	std::cout << "yamlize: validatedMappingTraits" << std::endl;
  if (has_FlowTraits<MappingTraits<T>>::value)
    io.beginFlowMapping();
  else
  {
	  std::cout << "beginMapping......" << std::endl;
    io.beginMapping();
  }
  if (io.outputting()) {
  ...
  if (!io.outputting()) {
	  std::cout << "no outputting......" << std::endl;
    StringRef Err = MappingTraits<T>::validate(io, Val);
    if (!Err.empty())
      io.setError(Err);
  }
...
yamlize(IO &io, T &Val, bool, Context &Ctx) {
	std::cout << "yamlize: unvalidateMappingTraits" << std::endl;
  if (has_FlowTraits<MappingTraits<T>>::value) {
    io.beginFlowMapping();
    detail::doMapping(io, Val, Ctx);
    io.endFlowMapping();
...
yamlize(IO &io, T &Val, bool, EmptyContext &Ctx) {
	std::cout << "yamlize: has_CustomMappingTraits" << std::endl;
  if ( io.outputting() ) {
    io.beginMapping();
...
yamlize(IO &io, T &Val, bool, EmptyContext &Ctx) {
	std::cout << "yamlize: missingTraits" << std::endl;
...
yamlize(IO &io, T &Seq, bool, Context &Ctx) {
	std::cout << "has_SequenceTraits: yamlize: 972" << std::endl;
	if ( has_FlowTraits< SequenceTraits<T>>::value ) {
		std::cout << "has_FlowTraits" << std::endl;
    ...
    io.endFlowSequence();
	  ...
	  std::cout << "not has_FlowTraits" << std::endl;
    unsigned incnt = io.beginSequence();
	std::cout << "incnt: " << incnt << std::endl;
    unsigned count = io.outputting() ? SequenceTraits<T>::size(io, Seq) : incnt;
    ...
    for(unsigned i=0; i < count; ++i) {
      ...
      if ( io.preflightElement(i, SaveInfo) ) {
		  std::cout << "preflightElement: true" << std::endl;
        yamlize(io, SequenceTraits<T>::element(io, Seq, i), true, Ctx);
		std::cout << "yamlize done" << std::endl;
...
operator>>(Input &yin, T &docList) {
	std::cout << "operator >> 1501" << std::endl;
  int i = 0;
  EmptyContext Ctx;
...
operator>>(Input &yin, T &docMap) {
	std::cout << "operator >> 1519" << std::endl;
  EmptyContext Ctx;
...
operator>>(Input &yin, T &docSeq) {
	std::cout << "operator >> 1532" << std::endl;
  EmptyContext Ctx;
  if (yin.setCurrentDocument())
  {
	std::cout << "begin yamlize" << std::endl;
...
operator>>(Input &In, T &Val) {
	std::cout << "operator >> 1544" << std::endl;
  EmptyContext Ctx;
...
operator>>(Input &In, T &Val) {
	std::cout << "operator >> 1556" << std::endl;
  EmptyContext Ctx;
  if (In.setCurrentDocument())
    yamlize(In, Val, true, Ctx);
  return In;
...
inline typename std::enable_if<missingTraits<T, EmptyContext>::value,
                               Input &>::type
operator>>(Input &yin, T &docSeq) {
	std::cout << "operator >> 1568" << std::endl;
  char missing_yaml_trait_for_type[sizeof(MissingTrait<T>)];
  return yin;
}
